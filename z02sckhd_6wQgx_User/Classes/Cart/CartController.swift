...
//        showHUD()
    ...
    /** 购物车 */
    @objc func load() {
        getRequest(baseUrl: Cart_URL, params: ["token" : "\(TMHttpUserInstance.sharedManager().member_id)"], success: { [weak self] (obj: CartInfo) in
            self?.hideAllHUD()
...
//            }
    ...
    func loadQuantity(num: Int, label: UILabel, indexPath: IndexPath) {
        ...
        getRequest(baseUrl: CartQuantity_URL, params: ["token" : "\(TMHttpUserInstance.sharedManager().member_id)", "spec_id" : "\(goods.spec_id)", "quantity" : "\(num)"], success: { [weak self] (obj: CartInfo) in
            self?.hideHUD()
            if "success" == obj.status {
    ...
    func loadCheck(indexPath: IndexPath, isSelected: Bool) {
        showHUD()
        let goods = stores[indexPath.section].result[indexPath.row]
        getRequest(baseUrl: CartCheck_URL, params: ["token" : "\(TMHttpUserInstance.sharedManager().member_id)", "rec_id" : "\(goods.rec_id)", "status" : isSelected ? "1" : "0"], success: { [weak self] (obj: BaseModel) in
            self?.hideHUD()
            if "success" == obj.status {
    ...
    func loadAllCheck(section: Int, sender: UIButton) {
        ...
        getRequest(baseUrl: CartAllCheck_URL, params: ["token" : "\(TMHttpUserInstance.sharedManager().member_id)", "status" : sender.isSelected ? "1" : "0", "sid" : "\(store.sid)"], success: { [weak self] (obj: BaseModel) in
            self?.hideAllHUD()
            if "success" == obj.status {
    ...
    func loadDelete(indexPath: IndexPath) {
        showHUD()
        let goods = stores[indexPath.section].result[indexPath.row]
        getRequest(baseUrl: CartDelete_URL, params: ["token" : "\(TMHttpUserInstance.sharedManager().member_id)", "rec_id" : "\(goods.rec_id)"], success: { [weak self] (obj: BaseModel) in
            self?.hideHUD()
            if "success" == obj.status {
...
//        sectionFooters[indexPath.section]?.title.text = "可获得\(store.give)个积分，可使用\(store.usable)个积分"
    ...
    @IBAction func pay() {
        showHUD()
        getRequest(baseUrl: CartSubmit_URL, params: ["token" : "\(TMHttpUserInstance.sharedManager().member_id)"], success: { [weak self] (obj: CartOrderInfo) in
            self?.hideHUD()
            if "success" == obj.status {
