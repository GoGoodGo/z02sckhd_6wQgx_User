...
class ApplyWithdrawController: TMViewController {
    ...
    /** 获取提现收益 */
    func load() {
        getRequest(baseUrl: Income_URL, params: ["token" : "\(TMHttpUserInstance.sharedManager().member_id)", "type" : "2"], success: { [weak self] (obj: IncomeInfo) in
            if "success" == obj.status {
    ...
    func loadWithdraw() {
        ...
        showHUD()
        getRequest(baseUrl: ApplyWithdraw_URL, params: ["token" : "\(TMHttpUserInstance.sharedManager().member_id)", "price" : textField.text!, "account_id" : accountID], success: { [weak self] (obj: BaseModel) in
            self?.hideHUD()
            if "success" == obj.status {
