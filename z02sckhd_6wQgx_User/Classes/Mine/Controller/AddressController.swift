...
class AddressController: TMViewController {
    ...
    func load() {
        showHUD()
        getRequest(baseUrl: Address_URL, params: ["token" : "\(TMHttpUserInstance.sharedManager().member_id)"], success: { [weak self] (obj: AddressInfo) in
            self?.hideHUD()
    ...
    func addAddress(name: String, phone: String, detial: String) {
        ...
        let params = ["token" : "\(TMHttpUserInstance.sharedManager().member_id)", "address" : address, "consignee" : name, "tel" : phone, "address_name" : detial]
        showHUD()
    ...
    func editAddress(name: String, phone: String, detial: String) {
        let address = province + " " + city + " " + district
        let params = ["aid" : aid, "token" : "\(TMHttpUserInstance.sharedManager().member_id)", "address" : address, "consignee" : name, "tel" : phone, "address_name" : detial]
        showHUD()
    ...
    /** 删除地址 */
    func delete() {
        getRequest(baseUrl: DelAddress_URL, params: ["aid" : aid, "token" : "\(TMHttpUserInstance.sharedManager().member_id)"], success: { [weak self] (obj: BaseModel) in
            if "success" == obj.status {
                self?.maskView.diss()
    ...
    func isDefault() {
        showHUD()
        getRequest(baseUrl: DefaultAddress_URL, params: ["address_id" : aid, "token" : "\(TMHttpUserInstance.sharedManager().member_id)"], success: { [weak self] (obj: BaseModel) in
            self?.hideHUD()
            if "success" == obj.status {
                self?.load()
            } else {
