...
class EvaluateController: TMViewController {
    ...
    /** 评价 */
    func load() {
        getRequest(baseUrl: Evaluate_URL, params: ["token" : "\(TMHttpUserInstance.sharedManager().member_id)", "rec_id" : "\((evaluate?.rec_id)!)", "comment" : comment, "Score" : score, "images" : imagesID], success: { [weak self] (obj: BaseModel) in
            self?.hideHUD()
