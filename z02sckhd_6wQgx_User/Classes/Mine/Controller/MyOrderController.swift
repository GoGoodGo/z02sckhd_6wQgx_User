...
class MyOrderController: TMViewController {
    ...
    @objc func load() {
        showHUD()
        getRequest(baseUrl: MyOrder_URL, params: ["token" : "\(TMHttpUserInstance.sharedManager().member_id)", "order_status" : status, "status" : evaluateStatus], success: { [weak self] (obj: MyOrderInfo) in
            self?.hideHUD()
    ...
    func loadEvaluateGoods() {
        showHUD()
        getRequest(baseUrl: NotEvaluate_URL, params: ["token" : "\(TMHttpUserInstance.sharedManager().member_id)"], success: { [weak self] (obj: NotEvaluateInfo) in
            self?.hideHUD()
            self?.tableView.mj_header.endRefreshing()
            if "success" == obj.status {
    ...
    func loadReceive(section: Int) {
        showHUD()
        getRequest(baseUrl: OrderReceive_URL, params: ["token" : "\(TMHttpUserInstance.sharedManager().member_id)", "order_id" : "\((orderInfo?.result[section]._orders.first?.order_id)!)"], success: { [weak self] (obj: BaseModel) in
            self?.hideHUD()
            if "success" == obj.status {
    ...
    func loadCancel(section: Int) {
        showHUD()
        getRequest(baseUrl: OrderCancel_URL, params: ["token" : "\(TMHttpUserInstance.sharedManager().member_id)", "mid" : "\((orderInfo?.result[section].mid)!)"], success: { [weak self] (obj: BaseModel) in
            self?.hideHUD()
            if "success" == obj.status {
                self?.orderInfo?.result.remove(at: section)
                self?.tableView.reloadData()
