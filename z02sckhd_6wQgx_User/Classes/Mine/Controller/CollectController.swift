...
class CollectController: TMViewController {
    ...
    private func setupUI() {
        ...
        collectionView.register(UINib.init(nibName: CellName(CollectGoodsCell.self), bundle: getBundle()), forCellWithReuseIdentifier: CellName(CollectGoodsCell.self))
        collectionView.register(UINib.init(nibName: CellName(CollectStoreCell.self), bundle: getBundle()), forCellWithReuseIdentifier: CellName(CollectStoreCell.self))
    ...
    @objc func load() {
        showHUD()
        getRequest(baseUrl: MyCollect_URL, params: ["token" : "\(TMHttpUserInstance.sharedManager().member_id)", "type" : "\(selectedIndex + 1)", "page" : "1"], success: { [weak self] (obj: CollectInfo) in
            self?.hideHUD()
    ...
    /** 加载更多 */
    @objc func loadMore() {
        getRequest(baseUrl: MyCollect_URL, params: ["token" : "\(TMHttpUserInstance.sharedManager().member_id)", "type" : "\(selectedIndex + 1)", "page" : "\(collectInfo?.data?.page ?? 2)"], success: { [weak self] (obj: CollectInfo) in
            if "success" == obj.status {
                self?.collectInfo?.data?.page += 1
    ...
    func loadCancel(indexPath: IndexPath) {
        ...
        showHUD()
        getRequest(baseUrl: CancelCollect_URL, params: ["token" : "\(TMHttpUserInstance.sharedManager().member_id)", "id" : "\((collect?.rec_id)!)"], success: { [weak self] (obj: BaseModel) in
            self?.hideHUD()
            if "success" == obj.status {
