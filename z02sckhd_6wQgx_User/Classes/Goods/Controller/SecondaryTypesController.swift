...
//        goodsCollectionView.mj_header = MJRefreshNormalHeader(refreshingTarget: self, refreshingAction: #selector(load))
    ...
    @objc func load() {
        ...
        showHUD()
        params = ["cate_id" : "\(currentCategory?.id ?? Int(pID)!)", "sort" : sort, "order" : order, "p" : "1","token" : "\(TMHttpUserInstance.sharedManager().member_id)"]
    ...
    /** 获取更多 */
    @objc func loadMore() {
        params = ["cate_id" : "\(currentCategory?.id ?? 0)", "sid" : "\(currentCategory?.sid ?? 0)", "sort" : sort, "order" : order, "p" : "\(page)","token" : "\(TMHttpUserInstance.sharedManager().member_id)"]
        getRequest(baseUrl: GoodsList_URL, params: params, success: { [weak self] (obj: DataInfo) in
            if "success" == obj.status {
                self?.page += 1
                if let data = obj.data {
